...
function buildConsignmentContractHtml(
  ...
  const itemFields = [
    ...
    field("Condition", contractDisplay(str(form.condition) || detail.condition)),
    field(
      "Inclusions",
      contractDisplay(str(form.inclusions) || detail.inclusions),
    ),
    field("Consignor's price", `<span class="price">${escapeHtml(price)}</span>`),
  ].join("");
...
async function openConsignmentContractPrintTab(
  detail: InquiryDetail,
): Promise<void> {
