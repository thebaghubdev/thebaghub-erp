...
export const DEFAULT_LAYAWAY_MONTHS = 3;
...
export function layawayMonthlyRateLabel(itemPrice: number): string {
  return itemPrice >= LAYAWAY_PRICE_THRESHOLD ? "1%" : "2%";
}

export function calculateLayawayPricing(
  itemPrice: number,
  months: number,
): { layawayPrice: number; monthlyPayment: number; monthlyRate: number } | null {
  ...
  return { layawayPrice, monthlyPayment, monthlyRate };
}

export function clampLayawayMonths(raw: string): string {
  if (raw.trim() === "") return "";
  const n = Number.parseInt(raw, 10);
  if (!Number.isFinite(n)) return "";
  return String(Math.min(MAX_LAYAWAY_MONTHS, Math.max(MIN_LAYAWAY_MONTHS, n)));
}
