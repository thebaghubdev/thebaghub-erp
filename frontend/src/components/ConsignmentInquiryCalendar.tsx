/**
 * Month grid for consignment inquiry calendar rows (INQ-SKUs inside day cells;
 * sidebar lists items for the selected day and links to inquiry details).
 */
import {
  addMonths,
  eachDayOfInterval,
...
export type ConsignmentInquiryCalendarKind =
  | "delivery_scheduled"
...
export type ConsignmentInquiryCalendarRow = {
  inquiryId: string;
  sku: string;
  itemLabel: string;
  consignorName: string;
  eventDate: string;
  kind: ConsignmentInquiryCalendarKind;
  status: string;
  isWalkIn: boolean;
};

type Props = {
  rows: ConsignmentInquiryCalendarRow[];
  isLoading?: boolean;
};

const WEEKDAYS = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"] as const;

const KIND_LABEL: Record<ConsignmentInquiryCalendarKind, string> = {
  delivery_scheduled: "For Delivery: Scheduled",
  delivery_rescheduled: "For Delivery: Rescheduled",
  processing: "For Processing",
  processing_walkin: "For Processing (Walk-in)",
  pullout_scheduled: "For Pullout: Scheduled",
  pullout_rescheduled: "For Pullout: Rescheduled",
};

const KIND_CHIP: Record<ConsignmentInquiryCalendarKind, string> = {
  delivery_scheduled:
    "border-green-500 bg-green-50 text-green-900 dark:border-green-400 dark:bg-green-950/50 dark:text-green-100",
  delivery_rescheduled:
    "border-purple-500 bg-purple-50 text-purple-900 dark:border-purple-400 dark:bg-purple-950/50 dark:text-purple-100",
  processing:
...
const KIND_SWATCH: Record<ConsignmentInquiryCalendarKind, string> = {
  delivery_scheduled: "bg-green-500",
  delivery_rescheduled: "bg-purple-500",
  processing: "bg-orange-500",
  processing_walkin: "bg-blue-500",
  pullout_scheduled: "bg-teal-500",
  pullout_rescheduled: "bg-purple-500",
};
...
const LEGEND: Array<{
  ...
  { kind: "delivery_scheduled", label: "Delivery scheduled" },
  { kind: "delivery_rescheduled", label: "Delivery rescheduled" },
  { kind: "processing", label: "Processing" },
  { kind: "processing_walkin", label: "Processing (walk-in)" },
  { kind: "pullout_scheduled", label: "Pullout scheduled" },
  { kind: "pullout_rescheduled", label: "Pullout rescheduled" },
];
...
function rowsByDayKey(rows: ConsignmentInquiryCalendarRow[]) {
  const m = new Map<string, ConsignmentInquiryCalendarRow[]>();
  for (const r of rows) {
    const k = dayKeyFromIso(r.eventDate);
    const list = m.get(k);
    if (list) list.push(r);
    else m.set(k, [r]);
  }
...
export function ConsignmentInquiryCalendar({ rows, isLoading }: Props) {
  const [viewMonth, setViewMonth] = useState(() => startOfMonth(new Date()));
  const [selectedDay, setSelectedDay] = useState(() => startOfDay(new Date()));

  const byDay = useMemo(() => rowsByDayKey(rows), [rows]);
  ...
  return (
    <div className="flex flex-col gap-6 xl:flex-row xl:items-start">
      <div className="min-w-0 flex-1">
        ...
        <ul className="mb-3 flex flex-wrap gap-x-3 gap-y-1.5 text-[0.7rem] text-slate-600 dark:text-slate-400">
          {LEGEND.map((item) => (
            <li key={item.kind} className="inline-flex items-center gap-1.5">
              <span
                className={`inline-block h-2 w-2 shrink-0 rounded-full ${KIND_SWATCH[item.kind]}`}
                aria-hidden
              />
              {item.label}
            </li>
                ...
                >
                  ...
                  <div className="flex min-h-0 flex-1 flex-col gap-0.5 overflow-hidden">
                    {dayRows.slice(0, 4).map((r) => (
                      <div
                        key={r.inquiryId}
                        className={`truncate rounded border-l-2 px-1 py-0.5 text-[0.6rem] font-mono font-medium leading-tight sm:text-[0.65rem] ${KIND_CHIP[r.kind]}`}
                        title={`${r.sku} · ${KIND_LABEL[r.kind]}`}
                      >
                        {r.sku}
                      </div>
                    ))}
        ...
        {rowsForSelectedDay.length === 0 ? (
          ...
          <ul className="mt-3 space-y-2">
            {rowsForSelectedDay.map((r) => (
              <li key={r.inquiryId}>
                <Link
                  to={`/portal/inquiries/${r.inquiryId}`}
                  aria-label={`Open inquiry ${r.sku}`}
                  className={`block rounded-lg border border-slate-200 border-l-4 bg-slate-50/80 px-3 py-2 text-sm transition-colors hover:bg-slate-100/90 focus-visible:outline focus-visible:ring-2 focus-visible:ring-violet-500 dark:border-slate-700 dark:bg-slate-950/50 dark:hover:bg-slate-800/60 ${sidebarBorderClass(r.kind)}`}
                >
                  <p className="break-all font-mono text-[0.65rem] font-semibold text-slate-800 dark:text-slate-100 sm:text-xs">
                    {r.sku}
                  </p>
