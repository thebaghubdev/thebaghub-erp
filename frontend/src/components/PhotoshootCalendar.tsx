/**
 * Month grid for item_photoshoot rows (SKUs inside day cells; sidebar lists
 * scheduled items for the selected day). Layout aligned with ConsignmentCalendar
 * but without branch filtering.
 */
import {
  addMonths,
  eachDayOfInterval,
...
export type PhotoshootCalendarRow = {
  id: string;
...
function photoshootsByDayKey(rows: PhotoshootCalendarRow[]) {
  const m = new Map<string, PhotoshootCalendarRow[]>();
  for (const r of rows) {
    const k = dayKeyFromIso(r.photoshootDate);
    const list = m.get(k);
    if (list) list.push(r);
    else m.set(k, [r]);
  }
...
export function PhotoshootCalendar({ rows, isLoading }: Props) {
  const [viewMonth, setViewMonth] = useState(() => startOfMonth(new Date()));
  const [selectedDay, setSelectedDay] = useState(() => startOfDay(new Date()));

  const byDay = useMemo(() => photoshootsByDayKey(rows), [rows]);
        ...
        {rowsForSelectedDay.length === 0 ? (
          ...
          <ul className="mt-3 space-y-2">
            {rowsForSelectedDay.map((r) => (
              <li key={r.id}>
                <Link
                  to={`/portal/photoshoot/item/${r.id}`}
                  aria-label={`Open photoshoot for ${r.sku}`}
                ...
                >
                  ...
                  <p
                    className="mt-1 line-clamp-2 whitespace-pre-wrap break-words text-xs text-slate-600 dark:text-slate-400"
                    title={r.inclusions !== "—" ? r.inclusions : undefined}
                  >
