...
function revokePreviews(previews: CheckPhotoPreview[]): void {
  for (const preview of previews) {
    if (preview.file) URL.revokeObjectURL(preview.previewUrl);
  }
}

function previewsFromSavedPhotos(photos: CheckPhotoKeyUrl[]): CheckPhotoPreview[] {
...
export function ConsignorPaymentCheckModal({
  ...
  const [photos, setPhotos] = useState<CheckPhotoPreview[]>([]);
  ...
  useEffect(() => {
    if (!open) return;
    setCheckNumber(initialCheckNumber ?? "");
    setPhotos(previewsFromSavedPhotos(initialPhotos));
    setDropActive(false);
    setFormError(null);
    onError(null);
  }, [open, initialCheckNumber, initialPhotos, onError]);
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    const trimmedNumber = checkNumber.trim();
    if (!trimmedNumber) {
      setFormError("Check number is required.");
      return;
    }
    if (photos.length === 0) {
      setFormError("At least one check photo is required.");
      return;
    }
    ...
    try {
      ...
      fd.append("checkNumber", trimmedNumber);
      ...
      const msg =
        err instanceof Error ? err.message : "Could not save check details";
      setFormError(msg);
      onError(msg);
    } finally {
      setBusy(false);
        ...
        <form onSubmit={(e) => void onSubmit(e)} className="mt-4 space-y-4">
          <div>
            ...
            <input
              ...
              maxLength={64}
              required
              className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500 disabled:bg-slate-100 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100"
            />
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              Upload one or more check images. Files are saved when you submit.
            </p>
            <input
              id={`${inputId}-photos`}
              type="file"
              ...
              multiple
              className="sr-only"
              aria-label="Select check photos"
              disabled={busy}
              onChange={(e) => {
            ...
            >
              <span className="text-sm font-medium text-slate-800 dark:text-slate-100">
                Drop check photos here or click to choose
              </span>
